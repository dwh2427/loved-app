  ...
  const onSubmit = async (data) => {
    try {
      ...
      const pageExists = pages?.find(
        (page) => page.username === selectedPage.username,
      );
      if (!pageExists) {
      ...
      if (tipAmount > 0) {
        ...
        if (token) {
          const result = await confirmPaymentIntent(
            ...
            email,
            tipAmount,
            selectedPage.currency,
            application_amount_fee,
            selectedPage.stripe_acc_id,
          );
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              >
                <div className="form-group relative">
                  <input
                    value={inputValue && toTitleCase(inputValue)}
                    {...register("inputValue")}
                    onChange={handleChange}
                    autoComplete="new-password"
                    className={`w-[330px] bg-[#F1F1F1]  p-3 pl-10 focus:outline-none ${filteredSuggestions.length > 0 ? "rounded-t-[25px] rounded-tl-[25px] rounded-tr-[25px]" : "rounded-full"}`}
                    type="text"
